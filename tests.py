...
class TestBooksCollector:
    ...
    def test_get_books_for_children_valid_value_book_is_for_children_by_genre(self, book_name, book_genre, collector):
        collector.add_new_book(book_name)
        collector.set_book_genre(book_name, book_genre)
    ...
    @pytest.mark.parametrize('book_name, book_genre',
                             [['Пикник на обочине', 'Фантастика'], ['451 градус по Фаренгейту', 'Ужасы'],
                              ['12 Стульев', 'Комедии']])
    def test_delete_book_from_favorites_valid_value_book_in_collection(self, book_name, book_genre, collector):
        collector.add_new_book(book_name)
        collector.set_book_genre(book_name, book_genre)
        collector.add_book_in_favorites(book_name)
